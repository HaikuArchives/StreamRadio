...
Expander::Draw(BRect updateRect)
{
	...
	shape.BezierTo(r.LeftTop() + BPoint(10, 0), r.LeftBottom() + BPoint(10, 0),
		r.LeftBottom() + BPoint(20, 0));
	shape.LineTo(r.RightBottom() - BPoint(20, 0));
	shape.BezierTo(r.RightBottom() - BPoint(10, 0),
		r.RightTop() - BPoint(10, 0), r.RightTop());

	SetHighColor(ui_color(B_CONTROL_BACKGROUND_COLOR));
	FillShape(&shape);

	SetHighColor(ui_color(
		IsFocus() ? B_CONTROL_HIGHLIGHT_COLOR : B_CONTROL_BORDER_COLOR));
	StrokeShape(&shape);
