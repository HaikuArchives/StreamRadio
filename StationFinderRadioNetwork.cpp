	...
	int capabilityIndex, const char* searchFor, BLooper* resultUpdateTarget)
{
	...
	if (data) {
		...
		xmlParserCtxt* parserCtxt = xmlCreatePushParserCtxt(
			NULL, NULL, xmlProlog, strlen(xmlProlog), "noname.xml");
		xmlParseChunk(parserCtxt, ((const char*) data->Buffer()),
			data->BufferLength(), 1);
		xmlDocPtr doc = parserCtxt->myDoc;
		xmlFreeParserCtxt(parserCtxt);
		for (xmlNode* nStation = doc->last->children; nStation;
			 nStation = nStation->next)
			if (strcmp((char*) nStation->name, "station") == 0) {
				Station* station = new Station("unknown");
				for (xmlAttr* prop = nStation->properties; prop;
					 prop = prop->next)
					if (strcmp((char*) prop->name, "name") == 0)
						station->SetName(
							(char*) XML_GET_CONTENT(prop->children));
						...
						fIconLookupList.AddItem(
							new IconLookup(station, iconUrl));
		...
		if (!fIconLookupList.IsEmpty()) {
			if (!fIconLookupThread)
				fIconLookupThread = spawn_thread(
					&IconLookupFunc, "iconlookup", B_NORMAL_PRIORITY, this);
...
const char* StationFinderRadioNetwork::baseUrl
