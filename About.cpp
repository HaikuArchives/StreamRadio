...
About::About()
	:
	BWindow(BRect(0, 0, 180, 50), B_TRANSLATE("About"), B_TITLED_WINDOW,
		B_NOT_ZOOMABLE | B_NOT_RESIZABLE)
{
	BRect bounds = Bounds();
	BStringView* versionView = new BStringView(bounds, "versionView",
		GetAppVersion(), B_FOLLOW_LEFT_RIGHT | B_FOLLOW_BOTTOM);
...
About::GetAppVersion()
{
	...
	if (be_app->GetAppInfo(&appInfo) == B_OK
		...
		&& appFile.InitCheck() == B_OK && appFileInfo.SetTo(&appFile) == B_OK
